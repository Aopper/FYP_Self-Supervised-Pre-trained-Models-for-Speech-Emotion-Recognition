...
#### parameters
test_speaker = config['test_speaker']
print("Processing test_speaker: ", test_speaker)
...
data_files = {
    "train": "/home/felix/Aopp/FINAL/data/EmoDB/{}/train.csv".format(test_speaker),
    "test": "/home/felix/Aopp/FINAL/data/EmoDB/{}/test.csv".format(test_speaker),
}
...
load_path = load_path + test_speaker
lr1 = config['lr1']
lr2 = config['lr2']
epoch = config['epoch']
...
def train_net(net, optimizer, train_iter, test_iter, start_epoch, num_epochs, device, losses, train_accs, test_accs, best_test_acc):
    ...
    if execution_time < 1:
        ...
        print(f"代码运行时间：{execution_time_ms:.2f} 毫秒")
    elif execution_time < 60:
        print(f"代码运行时间：{execution_time:.2f} 秒")
    else:
        execution_time_min = execution_time / 60
        print(f"代码运行时间：{execution_time_min:.2f} 分钟")
